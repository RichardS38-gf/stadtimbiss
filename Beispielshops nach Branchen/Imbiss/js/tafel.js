...
export function tafelReiter() {
  const leiste = document.querySelector('.tafel-reiter');
  if (leiste === null) return;

  const reiter = Array.from(leiste.querySelectorAll('.tafel-reiter__btn'));
  ...
  function zeigen(index) {
    reiter.forEach((btn, i) => {
      const aktiv = i === index;
      btn.classList.toggle('tafel-reiter__btn--aktiv', aktiv);
      btn.setAttribute('aria-selected', aktiv ? 'true' : 'false');
      btn.tabIndex = aktiv ? 0 : -1;
      gruppen[i].classList.toggle('tafel__gruppe--aktiv', aktiv);
    });
  }
